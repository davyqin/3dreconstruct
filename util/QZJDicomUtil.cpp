...
QZJDicomUtil::QZJDicomUtil()
:_pimpl(new Pimpl())
{
}

QZJDicomUtil::QZJDicomUtil(const std::string& fileName) 
:_pimpl(new Pimpl())
{
...
boost::shared_ptr<Image> QZJDicomUtil::fetchImage() const {
...
void QZJDicomUtil::setFileName(const string& filename) {
  _pimpl->fileName = filename;
}

boost::shared_ptr<unsigned short> QZJDicomUtil::pixel() {
...
int QZJDicomUtil::pixelLength() const {
  return _pimpl->nLength;
}

int QZJDicomUtil::imageHeight() const {
  return _pimpl->nRows;
}

int QZJDicomUtil::imageWidth() const {
  return _pimpl->nCols;
}

bool QZJDicomUtil::hasPixelData() const {
  return (_pimpl->pixelData.get() != 0);
}

void QZJDicomUtil::readFile()
{
